...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
  ...
  // Create a query to fetch the posts that have no parent (top-level tweets) (a thread that is not a comment/reply).
...
export async function addCommentToThread(
  ...
  try {
    ...
    const commentTweet = new Tweet({
      text: commentText,
      author: userId,
      parentId: tweetId, // Set the parentId to the original thread's ID
    });
